...
int coagulation_nucleation_surface()
{
    ...
    /*   //************************************************************ */
    // Declaration of variables
    int i,j,k,choice,counter,counter2,opt,printopt,flag,MAX,beta_option,restart_option;
    double v[MAX1+1],N[MAX1],X[MAX1][MAX1][MAX1],K[MAX1][MAX1],N1s[MAX1],sum1[MAX1],sum2[MAX1],temp,temp1,temp2,Vav,Vtotal;
    double t,step,temp3,Ninf,n,Vtot,addterm[MAX1],subterm[MAX1],Ntot,dpav,v1,fi,t_SPD,step_coag,q,P;
    double sigma,Ps,ns,theta,a,b,Jk,kstar,dpstar,vstar,s1,S,m1,c,zeta[MAX1],dp[MAX1],t1,t2,t3,t4,Kmin;
    double A,B,C,D,E,F,MW,rho,Ninf_eq,T,coolrate,d;
    double QM, Qsol, Ksp, viscosity, alpha, wetangle;
    double lambda,kn1,kn2,mu,D1,D2,m[MAX1],c1,c2,g1,g2,l1,l2,A_mu,B_mu;
    FILE *fptr,*fptr1,*fptr2,*fdout,*fbinsout,*frestart;
    if((fptr=fopen("main.inp","r+"))==NULL)//Opening the input file that contains property data for the aerosol material.
    {
        printf("Warning: Could not open file");
        return(-1);
    ...
    //Scanning the property data from the input file "main.inp"
    printf("File Read");
    fscanf(fptr,"Number of nodes: %d Starting Temperature(Kelvin): %lf Cooling Rate(Kelvin/s): %lf Concentration(M): %lf Molecular Weight(Kg/mol): %lf Density(Kg/m3): %lf Viscosity(Pa*s): %lf Surface Tension in the form A-BT(dyne/cm): A=%lf B=%lf Solubility Product Ksp: %lf Enter your choice - 1)Coagulation,2)nucleation+coagulation,3)nucleation + coagulation + surface growth,4)surface growth:%d Choice of collision frequency function:1)Free Molecular Regime,2)Fuchs form for free molecular and transition regime,3)Stokes-Einstein:%d Sutherland's constants for carrier gas in the form A*T^1.5/(B+T): A=%lf B=%lf Heterogeneous Wetting Angle: %lf Restart: %d",
           &MAX,&T,&coolrate,&QM,&MW,&rho,&viscosity,&A,&B,&Ksp,&choice,&beta_option,&A_mu,&B_mu,&wetangle,&restart_option);
    fclose(fptr);//Closing the main property data file.
    q=pow(10.0,(8.0/(MAX-2)));//Calculation of geometric spacing factor which depends on the number of nodes.
    v1 = MW/(rho*Na);// Volume of a monomer unit (a molecule); Na is the Avogadro's Number.

    if (restart_option == 1) {
        if((frestart=fopen("nucl-coag-surf-restart.dat","r+"))==NULL)//Opening the input file that contains restart info
        {
            printf("Warning: Could not open file");
            return(-1);
        }
    ...
    printf("\nTime\t\tN[1]\t\tJk\t\tS\t\tdiameter\tParticle Volume\tTotal Number\tTotal Volume");// Printing result heading.
    ...
    while(t<240.0)
    {
        ...
        if(t>0.14)
            step=1e-9;
        counter = counter +1;
        if(counter == 10000)// Printing mean diameter etc. after every 5,000 steps.
        {
            printf("\n%.9f\t%.6e\t%.6e\t%.6e\t%.6e\t%.6e\t%.6e",t,N[1],Jk,S,dpav,Vtotal,Ntot);
        ...
        // Moved beta options 1 and 2 to end of code for optimization...
        ...
        for(i=1;i<=MAX;i++)
        {
            ...
            //if (N1s[i] != N1s[i]) {printf("N1s[%i] is NaN",i);return(0);}
        }
        ...
        for(k=2;k<=MAX;k++)
        {
            ...
            if(step!=1e-5)
            {
                ...
                if( N[1]<N1s[k+1] )  // term2: evaporation of k+1 particle, gain in k, gain in monomer
                {
                    ...
                    {
                        addterm[k] = addterm[k] -(v[1]/(v[k+1]-v[k]))*K[1][k+1]*(N[1]-N1s[k+1])*N[k+1];//Growth of k due to evaporation of monomers from k+1.
                        t2 = t2 + K[1][k+1]*(-N[1]+N1s[k+1])*N[k+1];//Gain of monomers that have evaporated.
                        //if (t2 < 0){printf("\nt2 is %e, K is %e, N1 is %e, N1s is %e, Nk+1 is %e, k+1 is %i",t2,K[1][k+1],N[1],N1s[k+1],N[k+1],k+1);}
                    }
                }
                ...
                if(N[1]<N1s[k])  //term 4: evaporation from k particle, loss in k, gain in monomer (balanced in term1)
                {
                    if (k==2)
                    {
                        ...
                        //if (t2 < 0){printf("\nt2 is %e, K is %e, N1 is %e, N1s is %e, Nk is %e, k is %i",t2,K[1][k],N[1],N1s[k],N[k],k);}
                    }
                    else
                        subterm[k] = subterm[k] - (v[1]/(v[k]-v[k-1]))*K[1][k]*(N[1]-N1s[k])*N[k];
